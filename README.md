...
멀티모달 LLM(LLaVA)과 SAM 기반 비전 백본을 결합한 LISA 모델을 활용하여 물류 프로세스에서 분류 문제를 해결할 수 있는 텍스트-이미지 기반 모델 개발

암시적 텍스트 프롬프트를 이해할 수 있는 모델 학습을 위해 약 400여 개의 데이터셋을 구축

실제 프로세스에서 멀티모달 LLM을 효과적으로 활용할 수 있는 프레임워크 제안
...
ResNet, Faster R-CNN 기반 철강 제품 결함 및 객체 탐지 모델 구현

이미지 입력 시 결과를 시각화하는 데모 개발
...
상품 등록, 결제, 게시판 등 기능 포함한 Spring 기반 경매 웹 서비스 개발
